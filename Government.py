...
class GovernmentClass(ConsumerClass):
    ...
    def tax_revenue(self,opt=None):
        ...
        #Convert nested shares to quantities
    ...
    def revenue_and_utility(self,tau,goods=(2,)):
        ...
        #Translate the requested goods into the three tax rates
        ...
        #Solve (new) consumer problem
        opt = self.solve(do_print=False)

        #Calculate government revenue and consumer utility from this solution
    ...
    def max_revenue(self,goods=(2,),tau_max=10.0,N=1001):
        ...
        """
        #Grid over tax rate and resulting revenue
        tau_grid = np.linspace(0.0,tau_max,N)
        R_grid = np.empty(N)


        #Evaluate tax rates and find the one that achieves max revenue
        for i,tau_i in enumerate(tau_grid):
            R_grid[i],_ = self.revenue_and_utility(tau_i,goods)
    ...
    def find_tax_rate(self,R_target,goods=(2,),bracket=(1e-10,1.0)):
        ...
        """
        #Define goal when finding tax rate
        def objective(tau):
            R,u = self.revenue_and_utility(tau,goods)
            return R-R_target

        #Find the desired rate: R-R_target=0
        try:
